...
//cowntdown function. Set the date by modifying the date in next line (January 01, 2013 00:00:00):
		var austDay = new Date("June 26, 2013 00:00:00");
...
//function for the contact-form dropdown
	function contact() {
		if ($("#cform").is(":hidden"))
		{
			$("#ribbon").css({"background":"url(res/images/ribbon.png) bottom left no-repeat"});
			$("#home").slideUp("fast");
			$("#cform").slideDown("slow");
		}
		else{
			$("#ribbon").css({"background":"url(res/images/ribbon.png) top left no-repeat"});
			$("#cform").slideUp("slow");
			$("#home").slideDown("slow");
		}
	}
